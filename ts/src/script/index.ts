import * as ax from "uralsjs-axios-ajax"

export function addOverLayer(id: string|null = null) {
...
export function makeForm(html: string, id = '', header = '')
{
...
export function makePopupGallery(contentHtmlsArray: string[], id = '', index = 0)
{
...
export function switchGalleryContentRight(handler: HTMLElement)
{
...
export function switchGalleryContentLeft(handler: HTMLElement)
{
    const galleryContainer = handler.closest('.gallery-container');
    let targetShowId = parseInt(galleryContainer.getAttribute('data-show')) - 1;
    const galleryContents = Array.from(
...
export function switchGalleryContent(galleryContainer: HTMLElement)
{
...
export function ssrOverLayerForm(url, data = {}, id = '') {
...
export function ssrPopupGallery(url, data = {}, alerter = null, id = '') {
    const formData = new FormData();
    Object.keys(data).forEach(k => formData.append(k, data[k]));
    ax.sendDataAjax(
        ax.axiosBrowser(),
...
export function createAlert(text, style = 'danger')
{
...
export function errorReaction(response)
{
    if(response.error) {
...
export function onPressEnter(event, handler,  fn)
{
    if (event.key === "Enter") {
...
export function clickSubmitBtnInForm(handler)
{
